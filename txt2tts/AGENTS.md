...
app/
  ...
  services/
    ...
    llm_normalizer.py   # MiniMax M3（Anthropic Messages API 形态）
    tts_client.py       # 小米 MiMo（chat/completions 多模态音频）
...
- `LLM__*` —— MiniMax M3（Anthropic Messages API）：`API_KEY`（作为 `x-api-key` 头发送）、`BASE_URL=https://api.minimaxi.com/anthropic`、`MESSAGES_PATH=/v1/messages`、`MODEL=MiniMax-M3`、`API_VERSION=2023-06-01`、`MAX_TOKENS`、`TEMPERATURE`、`REQUEST_TIMEOUT_SEC`、`MAX_RETRIES`。响应取自 `content[0].text`。
...
- **`LlmSettings` 与 `TtsSettings` 各自负责请求体构造**（`build_request_body(...)`），修改 payload 字段时请集中在 `config.py` 调整。
...
- **M3 与 MiMo 是不同厂商、不同鉴权方式**：M3 使用 `x-api-key` + `anthropic-version` 头（Anthropic Messages API），MiMo 使用 `Authorization: Bearer`。混淆会导致其中一条链路静默失败。
...
- `app/config.py` —— 提示词、环境变量前缀、请求体的唯一权威来源。
- `app/main.py` —— 装配顺序（dotenv → settings → services → router configure → lifespan）。
- `run_e2e_xiaomi.py` —— 当 mock 测试与真实接口出现偏差时，对照此文件确认 M3 + MiMo 的真实请求形态。